// HTML 前端模板 (Vue 3 + Tailwind CSS 编译级净化版)
const UI_HTML = `
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    ...
    <title>站点保活管理系统 - Pro v1.1.0</title>
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700/80">
            <div>
                <h1 class="text-2xl font-extrabold flex items-center gap-2.5 text-gray-900 dark:text-white">
                    <span>🌐</span> Auto-Keepalive <span class="text-xs px-2.5 py-0.5 rounded-full bg-indigo-100 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-300 font-bold border border-indigo-200 dark:border-indigo-800">Pro v1.1.0</span>
                </h1>
            ...
            <div class="flex items-center gap-3 w-full md:w-auto justify-end">
                ...
                <button @click="saveConfig" class="bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-600 dark:hover:bg-indigo-500 text-white px-4 py-2 rounded-xl shadow-sm transition font-bold text-xs flex items-center gap-1.5">
                    <span>💾</span> 保存配置至服务器
                </button>
...
export default {
    async fetch(request, env, ctx) {
        ...
        const expectedAuth = 'Basic ' + btoa((env.ADMIN_USER || 'admin') + ':' + (env.ADMIN_PASS || 'admin'));
        
        if (!authHeader || authHeader !== expectedAuth) {
            return new Response('Unauthorized', { status: 401, headers: { 'WWW-Authenticate': 'Basic realm="Keepalive System Panel"' } });
        }
    ...
    async scheduled(event, env, ctx) {
        ...
        for (let task of config.tasks) {
            ...
            if (now - (task.lastCheck || 0) >= intervalMs) {
                ...
                try {
                    const res = await fetch(task.url, { method: 'GET', headers: { 'User-Agent': 'Cloudflare-KeepAlive-Pro/1.1.0' }, cf: { cacheTtl: 0 } });
                    isSuccess = res.ok;
